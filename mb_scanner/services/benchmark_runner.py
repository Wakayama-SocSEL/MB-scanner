"""ベンチマーク等価性チェックサービス

Node.jsスクリプトを使ってslow/fastコードの等価性を検証するサービスです。
"""

from concurrent.futures import ThreadPoolExecutor, as_completed
import json
from pathlib import Path
import subprocess

from mb_scanner.models.benchmark import EquivalenceResult, EquivalenceSummary

RUNNER_JS_PATH = Path(__file__).resolve().parent.parent / "resources" / "benchmark" / "runner.js"


def run_equivalence_check(
    entry_dir: Path,
    timeout: int = 100,
) -> EquivalenceResult:
    """単一エントリの等価性チェックを実行する

    Args:
        entry_dir: id_{id} ディレクトリのパス（slow.js / fast.js を含む）
        timeout: Node.js実行のタイムアウト秒数

    Returns:
        EquivalenceResult: チェック結果
    """
    dir_name = entry_dir.name
    entry_id = int(dir_name.replace("id_", ""))

    slow_path = entry_dir / "slow.js"
    fast_path = entry_dir / "fast.js"

    if not slow_path.exists() or not fast_path.exists():
        return EquivalenceResult(
            id=entry_id,
            status="error",
            comparison_method="none",
            error_message=f"Missing slow.js or fast.js in {entry_dir}",
        )

    try:
        proc = subprocess.run(
            ["node", str(RUNNER_JS_PATH), str(slow_path), str(fast_path), str(timeout * 1000)],
            capture_output=True,
            text=True,
            timeout=timeout + 5,  # Node.js内部タイムアウト + マージン
            check=False,
        )

        if proc.returncode != 0:
            return EquivalenceResult(
                id=entry_id,
                status="error",
                comparison_method="none",
                error_message=proc.stderr.strip() or "Node.js process exited with non-zero code",
            )

        result_data: dict[str, str | None] = json.loads(proc.stdout.strip())
        return EquivalenceResult(
            id=entry_id,
            status=result_data["status"],  # type: ignore[arg-type]
            slow_output=result_data.get("slow_output"),
            fast_output=result_data.get("fast_output"),
            comparison_method=result_data["comparison_method"],  # type: ignore[arg-type]
            error_message=result_data.get("error_message"),
        )

    except subprocess.TimeoutExpired:
        return EquivalenceResult(
            id=entry_id,
            status="timeout",
            comparison_method="none",
            error_message=f"Timed out after {timeout} seconds",
        )
    except json.JSONDecodeError as e:
        return EquivalenceResult(
            id=entry_id,
            status="error",
            comparison_method="none",
            error_message=f"Failed to parse Node.js output: {e}",
        )


def run_batch_equivalence_check(
    input_dir: Path,
    target_ids: set[int] | None = None,
    count: int | None = None,
    offset: int = 0,
    timeout: int = 100,
    workers: int = 4,
) -> EquivalenceSummary:
    """バッチで等価性チェックを実行する

    Args:
        input_dir: id_* ディレクトリを含む親ディレクトリ
        target_ids: チェック対象のIDセット（Noneで全件）
        count: チェックする最大件数
        offset: 開始位置（ソート後のインデックス）
        timeout: 1件あたりのタイムアウト秒数
        workers: 並列ワーカー数

    Returns:
        EquivalenceSummary: 全体のサマリー
    """
    # id_* ディレクトリを検索してソート
    entry_dirs = sorted(
        [d for d in input_dir.iterdir() if d.is_dir() and d.name.startswith("id_")],
        key=lambda d: int(d.name.replace("id_", "")),
    )

    # フィルタリング
    if target_ids is not None:
        entry_dirs = [d for d in entry_dirs if int(d.name.replace("id_", "")) in target_ids]

    # offset / count 適用
    entry_dirs = entry_dirs[offset:]
    if count is not None:
        entry_dirs = entry_dirs[:count]

    # 並列実行
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(run_equivalence_check, entry_dir, timeout): entry_dir for entry_dir in entry_dirs}
        results = [future.result() for future in as_completed(futures)]

    # IDでソートして結果の順序を保証
    results.sort(key=lambda r: r.id)

    # サマリー集計
    status_counts = {"equal": 0, "not_equal": 0, "error": 0, "timeout": 0, "skipped": 0}
    for r in results:
        status_counts[r.status] += 1

    return EquivalenceSummary(
        total=len(results),
        equal=status_counts["equal"],
        not_equal=status_counts["not_equal"],
        error=status_counts["error"],
        timeout=status_counts["timeout"],
        skipped=status_counts["skipped"],
        results=results,
    )
