...
- `mb-analyzer/src/pruning/rules/blacklist.ts`: `getGrammarBlacklist()` が module-level lazy cache で 3 カテゴリ (statement / identifier / expression) 別に `Map<parentType, Map<key, ExcludeRule>>` を返す
