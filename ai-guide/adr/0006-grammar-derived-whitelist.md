...
4. **コード変更は `constants.ts` 1 ファイルに局所化**。`categories.ts:HANDLERS` / `replace.ts` / `engine.ts` 全て無改修で動く
...
```
parser.ts:plugins         constants.ts (whitelist)
─────────────────────────────────────────────────────
[]                         TS/JSX/Flow を全除外 (← 現状)
...
各 parser plugin を有効化することで対象に追加できる dataset を整理する。実装変更は `parser.ts:plugins` への追加と `constants.ts` の除外群解除を **paired** で行う。
...
- `t.FLIPPED_ALIAS_KEYS` の semi-public API 依存 — ADR-0005 と同じ Babel 内部 API リスク。Babel メジャーバージョン更新時の cross-check は ADR-0005 の枠組みを再利用

## トリガー (再検討の条件)

以下のいずれかが成立したら本 ADR を見直す:

- **対象言語が拡張される** — TS / Flow / JSX を含む dataset を扱う必要が生じた場合、`parser.ts:plugins` への有効化と `constants.ts` の除外群解除を **paired** で行う。本 ADR 本体を改訂し、§対象言語拡張で扱える dataset 例 に該当エントリを追記する (新規 ADR は不要)
