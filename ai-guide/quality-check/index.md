...
| 外部コマンド実行（`subprocess`、`node dist/cli.js`） | ドメインモデル（Pydantic / shared/types.ts） |
...
`mb_scanner/domain/entities/equivalence.py` と `mb-analyzer/src/shared/types.ts` の型定義は JSON 契約で一致している必要がある（詳細は [`../architecture/index.md`](../architecture/index.md)）。これを壊さないため:

- Python 側: `tests/adapters/gateways/equivalence/` に subprocess を介した integration test を配置し、`dist/cli.js` を実機呼び出しで検証する
- TypeScript 側: `tests/shared/types.ts` で Python 側 Pydantic 互換の型ガードを確認、`tests/cli/` で stdin/stdout 経由の E2E を確認する
