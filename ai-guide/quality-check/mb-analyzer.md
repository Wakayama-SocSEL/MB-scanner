...
| 区分 | 配置 | 例 |
|---|---|---|
| **公開 API** (export されたシンボル) | `tests/...test.ts` | `enumerateCandidates` のテスト → `tests/pruning/candidates.test.ts` |
| **モジュール内部ヘルパ** (export していないシンボル) | 同一 `src/` ファイル末尾の `if (import.meta.vitest)` ブロック | `isCandidate` / `walkAst` のテスト → `src/pruning/candidates.ts` 内 |
