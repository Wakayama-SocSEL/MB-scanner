...
## 配置規約: 公開 API は `tests/`、内部ヘルパは in-source (ADR-0007)

| 区分 | 配置 | 例 |
|---|---|---|
| **公開 API** (export されたシンボル) | `tests/...test.ts` | `enumerateCandidates` のテスト → `tests/pruning/ast/candidates.test.ts` |
| **モジュール内部ヘルパ** (export していないシンボル) | 同一 `src/` ファイル末尾の `if (import.meta.vitest)` ブロック | `isCandidate` / `walkAst` のテスト → `src/pruning/ast/candidates.ts` 内 |

判断ルールは **export の有無** だけ。複雑度や規模で振り分けない (主観で揺れて二重規範化する)。詳細根拠は [ADR-0007](../adr/0007-in-source-testing-internal-helpers.md)。
