...
from mb_scanner.adapters.gateways.github.search import SearchCriteria


def test_github_client_initialization_with_token():
    """GitHubClientがトークン付きで正しく初期化されることを確認する"""
    # Arrange & Act
    with patch("mb_scanner.adapters.gateways.github.client.Github"):
...
def test_github_client_initialization_without_token():
    ...
    # Arrange & Act & Assert
    with patch("mb_scanner.adapters.gateways.github.client.settings") as mock_settings:
...
def test_github_client_search_repositories():
    ...
    # GitHubClientをモック
    with (
        patch("mb_scanner.adapters.gateways.github.client.Github") as mock_github_class,
        patch("mb_scanner.adapters.gateways.github.client.cast") as mock_cast,
    ):
...
def test_github_client_search_repositories_github_exception():
    ...
    # Arrange
    with patch("mb_scanner.adapters.gateways.github.client.Github") as mock_github_class:
        mock_github_instance = Mock()
        mock_github_class.return_value = mock_github_instance
...
def test_github_client_search_repositories_with_default_criteria():
    ...
    with (
        patch("mb_scanner.adapters.gateways.github.client.Github") as mock_github_class,
        patch("mb_scanner.adapters.gateways.github.client.cast") as mock_cast,
        patch("mb_scanner.adapters.gateways.github.client.build_default_search_criteria") as mock_build_default,
    ):
        mock_github_instance = Mock()
        mock_github_class.return_value = mock_github_instance
